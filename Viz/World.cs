...
namespace attplan.Viz
{
    public class World
    {
        ...
        public MoonWithMetadata Moon;
        public string SpiceDir;
        public StarBackground Stars;
        public TexturedBall Sun;
        public Vector3d TempVector;
        public TrajectoryShape Trajectory;
        public PointShape LookaroundPoint = new PointShape { Visible = true };
        private long _time;

        public bool DrawGrazingPoint = false;
        ...
        public void Tick()
        {
            ...
            //Fetcher.FetchPositionAndAttitude(LadeeStateFetcher.LadeeId, _time, ref LADEE.Position, ref LADEE.Attitude);

            Fetcher.FetchLADEEPositionVelocityAndAttitude(_time, ref BioSentinel.Position, ref BioSentinel.Attitude, ref BioSentinel.VelocityVector.Vector);
            ...
            var moonVec = Moon.Position - BioSentinel.Position;
            moonVec.NormalizeFast();
            BioSentinel.MoonVector.Vector = moonVec;
            var earthVec = Earth.Position - BioSentinel.Position;
            earthVec.NormalizeFast();
            BioSentinel.EarthVector.Vector = earthVec;
            var sunVec = Sun.Position - BioSentinel.Position;
            sunVec.NormalizeFast();
            BioSentinel.SunVector.Vector = sunVec;

            LadeeVelocityMeNormalized = LadeeVelocityMe;
            LadeeVelocityMeNormalized.Normalize();

            if (success)
                UpdateMoonLOD(_time);

            if (Moon.IsMetadataVisible)
                Moon.UpdateGrazingPointInfo(this, _time);
    ...
    {
        public Vector3 Point1;  // Units are kilometers * KilometersF
