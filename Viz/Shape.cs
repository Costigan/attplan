...
namespace attplan.Viz
{
    public abstract class Shape
    {
        ...
        public const float MetersF = 0.000001f;
        public const float KilometersF = 0.001f;
        ...
        {
            var a = new Vector3((float)x1, (float)y1, (float)z1);
            var b = new Vector3((float)x2, (float)y2, (float)z2);
            DrawLine(a, b, color);
        }

        public void DrawLine(LineSegment seg, Color color)
        {
            DrawLine(seg.Point1, seg.Point2, color);
        }

        public void DrawPoint(Vector3 p, Color color, float size = 8f)
        {
            GL.Disable(EnableCap.Lighting);
            GL.Color3(color);
            GL.PointSize(size);
